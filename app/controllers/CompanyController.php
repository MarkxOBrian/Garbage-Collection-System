   ...
   $dbpath = ''; 
    ...
    if (isset($_GET['add2'])) {
         $compyname = $_POST['compname'];
            ...
            $sql = "INSERT INTO companies (name, email, category, location, details, phoneno, password) VALUES (?, ?, ?, ?, ?, ?, ?)";
            $stmt = $db->prepare($sql);
            $stmt->bind_param("sssssss", $compyname, $comemail, $compcat, $clocation, $compdet, $cphonenos, $hashed);
            $stmt->execute();
